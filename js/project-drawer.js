  ...
  const open = id => {
    ...
    if (isPilotBase(project)) {
      if (body) {
        body.innerHTML = renderHeader(project, records)
          ...
          + renderBaseOverview(project, baseData.bases[String(project.序号)] || {});
        body.scrollTop = 0;
      }
    ...
    if (body) {
      body.innerHTML = renderHeader(project, records)
        ...
        + '<section class="pd-section pd-readonly-note"><h3>项目进展</h3><p>项目进展由工作台统一填报，当前页面仅提供历史记录浏览。</p></section>';
    }
